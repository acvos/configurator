...
describe("File reader", () => {
  it("can read JSON", async () => {
    const container = await configurator.load([{ type: "file", value: "test/fixtures/test-config.json" }])
  ...
  it("can read YAML", async () => {
    const container = await configurator.load([{ type: "file", value: "test/fixtures/test-config.yaml" }])
  ...
  it("can resolve glob patterns", async () => {
    const container = await configurator.load([{ type: "file", value: "**/test-config.{yaml,json}" }])
